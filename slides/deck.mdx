...
```notes
I'm going to talk about these topic
So what does these three things have to do with each other?
```
...
TODO RS Check why Bob still gets money and no money deducted from Alice
```
...
What if we leave them out? What is the default?
```
...
```notes
`koa-cookie` does not enable `sameSite` cookie by default
```
...
```notes
Cookies used to track user sessions
Good cookies headers like httpOnly, secure, and sameSite

Attack method 1: XSS can cause cookie exfil. Sanitise/validate your input
Attack method 2: CSRF is when an action is performed on behalf of users using cookies
