...
- With encoding, browser sees `&lt;script&gt;`

```notes
Type of injection,
Malicious browser side scripts are injected into trusted websites.
Flaws occur anywhere a web application uses input from a user without validating or encoding it.
```
...
- CORS allow other origins to load resources
...
```notes
ACAO headers do not allow list, it is all or one or nothing
```
...
I also want credentials (cookies) sent
